...
const char * const BITCOIN_CONF_FILENAME = "executecoin.conf";
const char * const BITCOIN_PID_FILENAME = "executecoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "executecoin" is a composite category enabling all ExecuteCoin-related debug output
                if(ptrCategory->count(std::string("executecoin"))) {
...
#else
    const char* pszModule = "executecoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ExecuteCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ExecuteCoin
    // Mac: ~/Library/Application Support/ExecuteCoin
    // Unix: ~/.executecoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ExecuteCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ExecuteCoin";
#else
    // Unix
    return pathRet / ".executecoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty executecoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override executecoin.conf
